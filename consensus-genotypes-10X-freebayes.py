...
##FORMAT=<ID=DI,Number=R,Type=Integer,Description="Allele observation counts from Illumina">
##FORMAT=<ID=DX,Number=R,Type=Integer,Description="Allele observation counts from 10X">
##FORMAT=<ID=QI,Number=R,Type=Integer,Description="Sum of qualities of allele observations from Illumina">
##FORMAT=<ID=QX,Number=R,Type=Integer,Description="Sum of qualities of allele observations from 10X">
	...
	for variant_table in vcf_reader_freebayes:
		...
		for vt, var2index in variant_tables_10x:
			for sample in vt.samples:
				...
				ad = vt.allele_depths_of(sample)
				aqs = vt.allele_quality_sums_of(sample)
				i = samples2index[sample]
				genotypes_10x[i] = (g, ad, aqs, var2index)
		...
		for variant_index, variant in enumerate(variant_table.variants):
			...
			for sample_index, sample in enumerate(samples):
				...
				if gt_freebayes == -1:
					keep_variant = False
				gt_list_10x, ad_list_10x, aqs_list_10x, var2index10x = genotypes_10x[sample_index]
				if variant in var2index10x:
					idx_10x = var2index10x[variant]
					gt_10x = gt_list_10x[idx_10x]
				else:
					idx_10x = None
					gt_10x = -2
				if (gt_freebayes == 0) and (gt_10x < 0):
					...
					format_fields.append([gt2string[gt_freebayes], ad_freebayes, ad_list_10x[idx_10x], aqs_freebayes, aqs_list_10x[idx_10x]])
				else:
					keep_variant = False
